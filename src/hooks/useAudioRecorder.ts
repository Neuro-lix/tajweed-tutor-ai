...
interface UseAudioRecorderReturn {
  ...
  stopRecording: () => Promise<{ base64: string; mimeType: string; blob: Blob } | null>;
  error: string | null;
}
...
export const useAudioRecorder = (): UseAudioRecorderReturn => {
  ...
  const stopRecording = useCallback(async (): Promise<{ base64: string; mimeType: string; blob: Blob } | null> => {
    return new Promise<{ base64: string; mimeType: string; blob: Blob } | null>((resolve) => {
      ...
      if (recorder && isRecording) {
        recorder.onstop = async () => {
          ...
          reader.onloadend = () => {
            ...
            cleanup();
            resolve({ base64: base64Data, mimeType, blob });
          };
          reader.onerror = () => {
