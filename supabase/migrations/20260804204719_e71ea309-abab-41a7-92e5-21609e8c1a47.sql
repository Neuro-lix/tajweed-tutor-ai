...
CREATE OR REPLACE FUNCTION private.get_public_leaderboard()
RETURNS TABLE (
  id uuid,
  display_name text,
  total_xp integer,
...
REVOKE ALL ON FUNCTION private.get_public_leaderboard() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.get_public_leaderboard() TO anon, authenticated, service_role;

DROP VIEW IF EXISTS public.leaderboard_public;
CREATE VIEW public.leaderboard_public
WITH (security_invoker = on) AS
SELECT * FROM private.get_public_leaderboard();
